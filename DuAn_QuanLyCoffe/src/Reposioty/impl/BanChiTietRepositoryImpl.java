...
import Reposioty.BanChiTietRepository;

/**
 *
 * @author HP
 */
public class BanChiTietRepositoryImpl implements BanChiTietRepository{
    ...
    public void insert(BanChiTiet Entity) {
        Helper.SQLConlection.update(INSERT_SQL, Entity.getID_Ban(), Entity.getID_HoaDon(), Entity.getThoidiemCoNguoi(), Entity.isBanChinh());
    }

    @Override
    public void update(int idban,int idHoadon) {
        Helper.SQLConlection.update(UPDATE_SQL, idban,idHoadon);
    }

    @Override
    public void delete(int id) {
        Helper.SQLConlection.update(DELETE_SQL, id);
    }
    ...
    public List<Integer> selectallbangop(){
        ...
        try {
            ResultSet rs = SQLConlection.query(SELECT_ALL_BAN_GOP);
            while (rs.next()) {
