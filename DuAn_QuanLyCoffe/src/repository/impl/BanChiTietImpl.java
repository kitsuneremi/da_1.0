 ...
 * @author DELL
 */
public class BanChiTietImpl implements BanChiTietRepository{
    ...
    public void insert(BanChiTiet Entity) {
        SQLCollection.update(INSERT_SQL, Entity.getID_Ban(), Entity.getID_HoaDon(), Entity.getThoidiemCoNguoi(), Entity.isBanChinh());
    }

    @Override
    public void update(int idban,int idHoadon) {
        SQLCollection.update(UPDATE_SQL, idban,idHoadon);
    }
    ...
    public List<Integer> selectallbangop(){
        ...
        try {
            ResultSet rs = SQLCollection.query(SELECT_ALL_BAN_GOP);
            while (rs.next()) {
